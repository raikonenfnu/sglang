...
class ServerArgs:
    ...
    def _handle_model_specific_adjustments(self):
        ...
        if model_arch in [
            ...
            # The kernel expects the SEPARATED gate/up tile layout (gate rows
            # followed by up rows) — matching both the FlyDSL tuned entries
            # and the de-interleave that the GPT-OSS Quark loader applies —
            # while the runner default for swiglu paths is INTERLEAVE.
            # Detect it from the Quark `global_quant_config` here (mirrors
            # `_is_mx_w4a8` in quark.py) so we can flip the layout env below
            # without leaking global env mutation into the scheme.
            ...
            if is_hip() and envs.SGLANG_USE_AITER.get() and (is_mxfp4_quant_format or is_quark_w4a8_mxfp4_format):
                ...
                logger.warning(
                    "Detected ROCm and Quark W4A8 MXFP4-FP8 quantization "
                    "format for GPT-OSS model, forcing SEPARATED gate/up "
                    "layout in the aiter fused-MoE kernel."
                )
